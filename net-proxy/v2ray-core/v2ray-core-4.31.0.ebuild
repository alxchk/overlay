...
EGO_PN="github.com/v2ray/v2ray-core"
...
"github.com/pires/go-proxyproto v0.2.0/go.mod"
...
"golang.org/x/sys v0.0.0-20201006155630-ac719f4daadf/go.mod"
...
src_prepare() {
    eapply_user
    sed -i "s@/usr/local/bin@${ESYSROOT}/usr/bin@;s@/usr/local/etc@${ESYSROOT}/etc@" \
	release/config/systemd/system/v2ray@.service
}

src_compile() {
    env GOBIN="${S}/bin" \
	go build -mod=readonly -o v2ray ./main || die "compile failed"

    env GOBIN="${S}/bin" \
	go build -mod=readonly -o v2ctl ./infra/control/main || die "compile failed"
}
...
src_install() {
    ...
    doins release/config/systemd/system/v2ray@.service

    insinto /lib/systemd/user
    doins release/config/systemd/system/v2ray@.service
    
    insinto /etc/v2ray
    doins release/config/{config.json,geoip.dat,geosite.dat,vpoint_socks_vmess.json,vpoint_vmess_freedom.json}
}
